...
export function fromDbRecord(row: PrismaChildProfile): ChildProfile {
  if (row.snapshotJson) {
    try {
      ...
      return withChildDefaults({
        ...
        schoolYear: parsed.schoolYear ?? row.yearGroup ?? parsed.yearGroup,
        keyStageLevel: parsed.keyStageLevel ?? "",
        subjectLevel: parsed.subjectLevel ?? "",
        learningGoals: parsed.learningGoals ?? [],
        senSupportNeeds: parsed.senSupportNeeds ?? "",
        ageRange: parsed.ageRange ?? normalizeAgeRange(row.yearGroup),
        yearGroup: row.yearGroup ?? parsed.yearGroup,
        ageYears: parsed.ageYears ?? row.age ?? 6,
        stars: row.stars,
        xp: row.xp,
        coins: row.coins,
  ...
  return withChildDefaults({
    ...
    avatar: row.avatar ?? "🦊",
    dateOfBirth: null,
    schoolYear: row.yearGroup ?? "Year 1",
    keyStageLevel: "",
    subjectLevel: "",
    learningGoals: [],
    senSupportNeeds: "",
