      ...
      <div className="max-h-[calc(100dvh-1rem)] w-full max-w-6xl overflow-y-auto rounded-2xl border border-slate-700 bg-slate-950 p-3 sm:max-h-[calc(100dvh-2rem)] sm:p-6">
        <div className="mb-4 flex items-start justify-between gap-3">
          ...
          <div className="flex shrink-0 items-center gap-2">
            <button type="button" disabled={academicRevalidating} onClick={() => void revalidateImportedLesson()} className="rounded-lg border border-cyan-500/40 px-3 py-1.5 text-xs font-bold text-cyan-100 hover:bg-cyan-500/10 disabled:opacity-50">
              {academicRevalidating ? "Revalidating…" : "Revalidate imported content"}
            </button>
            <button type="button" onClick={onClose} className="text-sm font-bold text-slate-400 hover:text-white">✕</button>
          </div>
        </div>
