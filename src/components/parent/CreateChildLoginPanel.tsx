...
export default function CreateChildLoginPanel({ childId, childName, onSuccess, onCancel }: Props) {
  ...
  }, [childName, loginMode, manualUsername]);
    ...
    <form onSubmit={(event) => void handleSubmit(event)} className="space-y-4 rounded-2xl border border-cyan-500/30 bg-cyan-500/5 p-4">
          ...
          {usernameCheck ? (
            <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {usernameCheck.suggestions.map((suggestion) => (
                    <button
                      ...
                      className="rounded-lg border border-cyan-500/40 bg-cyan-500/10 px-2.5 py-1 text-xs font-semibold text-cyan-200 hover:bg-cyan-500/20"
                    >
                      {suggestion}
                    </button>
                  ))}
              ...
              <p className="text-xs text-slate-500">
                You can still type a different username manually if you prefer.
              </p>
            </div>
          ) : null}
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        <Button
          ...
          disabled={
            saving
            || (loginMode === "manual" && usernameCheck !== null && usernameCheck.valid && !usernameCheck.available)
          }
