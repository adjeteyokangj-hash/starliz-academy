...
import { approveGaAudioAsset, serializeGaAudioAsset } from "@/lib/ga-audio";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const asset = await approveGaAudioAsset(body, session.userId);
    if (!asset) return NextResponse.json({ error: "Ga audio asset not found." }, { status: 404 });
    await writeAuditLog({
      actorUserId: session.userId,
      action: "ga_audio_asset.approved",
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to approve Ga audio asset." }, { status: 400 });
  }
}
