...
import { linkExistingChildLoginAccount } from "@/lib/child-account-create";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (session.role !== "parent") {
    return NextResponse.json(
      { error: "Only parent accounts can create child logins.", code: "forbidden" },
      { status: 403 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json({ error: "Invalid Create Login request." }, { status: 400 });
  }
  ...
  void writeAuditLog({
    ...
    action: "child_login_linked_existing",
