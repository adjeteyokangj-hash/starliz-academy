...
function makeDeps(overrides: {
  ...
  adminProfile?: { id: string; roleId: string | null } | null;
  permission?: boolean;
  publicUrl?: string;
} = {}) {
  ...
  return {
    deps: {
      ...
      findAdminUser: async () => overrides.adminProfile ?? { id: "admin-profile-1", roleId: null },
      hasPermission: async () => overrides.permission ?? true,
...
test("admin message upload rejects unauthenticated users", async () => {
  ...
  const response = await handleAdminMessageUpload(uploadRequest(file), deps);
...
test("admin message upload rejects admins without messaging permission", async () => {
  const { deps, calls } = makeDeps({
    adminProfile: { id: "admin-profile-1", roleId: "limited-role" },
    permission: false,
  });
  const file = new File(["image-bytes"], "photo.jpg", { type: "image/jpeg" });
