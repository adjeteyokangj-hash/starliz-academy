...
const CSV_PATH = resolve(__dirname, "ga-foundation-batch-2.csv");
...
try {
  ...
  if (!source) {
    source = await prisma.gaSource.create({
      data: {
        ...
        sourceYear: 2025,
        section: "English-Ga",
        notes: "Seeded from batch 2 import script",
      },
      select: { id: true, sourceName: true },
    });
  }
  ...
  for (const row of rows) {
    ...
    const finalReviewStatus = existing?.reviewStatus === "Approved" && desiredReviewStatus !== "Approved"
      ? "Approved"
      : desiredReviewStatus;
