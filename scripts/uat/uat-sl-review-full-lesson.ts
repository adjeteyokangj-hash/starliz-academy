...
} from "../../src/lib/schools/parse-playable-lesson-content.ts";
...
async function main() {
  ...
  if (block.contentId !== CONTENT_ID) {
    throw new Error(`Unexpected contentId ${block.contentId}`);
  }
  ...
  // English sample if present in DB
  ...
  const checks = {
    objective: parsed.ok && parsed.learningObjective?.includes("multiplication"),
    ...
    englishOk: englishParsed ? englishParsed.ok && Boolean(englishParsed.ok && englishParsed.passage?.text) : null,
